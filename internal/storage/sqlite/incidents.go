...
func (t *txStore) UpdateIncident(ctx context.Context, v domain.Incident, expected int64) error {
	result, err := t.tx.ExecContext(ctx, `UPDATE incidents SET status=?,commander_id=?,closed_at=?,version=version+1,updated_at=? WHERE id=?`, v.Status, v.CommanderID, nullableStamp(v.ClosedAt), stamp(v.UpdatedAt), v.ID, expected)
	if err != nil {
