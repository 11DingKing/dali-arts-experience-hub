...
func (s *Store) ClaimOutbox(ctx context.Context, now time.Time, limit int) ([]domain.OutboxEvent, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id,topic,aggregate_type,aggregate_id,payload,status,attempts,next_attempt_at,last_error,created_at,updated_at FROM outbox_events WHERE status='pending' AND next_attempt_at<=? ORDER BY next_attempt_at,id LIMIT ?`, stamp(now), limit)
	if err != nil {
