...
func (t *txStore) UpdateTeam(ctx context.Context, v domain.Team, expected int64) error {
	result, err := t.tx.ExecContext(ctx, `UPDATE teams SET status=?,current_incident_id=?,leader_id=?,version=version+1,updated_at=? WHERE id=?`, v.Status, nullText(v.CurrentIncidentID), v.LeaderID, stamp(v.UpdatedAt), v.ID, expected)
	if err != nil {
