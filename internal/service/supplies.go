...
func (c *Coordinator) AllocateSupply(ctx context.Context, cmd domain.AllocateSupplyCommand) (domain.Allocation, error) {
	...
	err := c.repo.WithinTx(txCtx, func(tx repository.Tx) error {
		lot, err := tx.GetSupplyLot(txCtx, cmd.LotID)
		if err != nil {
			return err
		}
		task, err := tx.GetTask(txCtx, cmd.TaskID)
		if err != nil {
			return err
		}
		...
		lot.UpdatedAt = now
		if err := tx.UpdateSupplyLot(txCtx, lot, cmd.LotVersion); err != nil {
			return err
		}
		if err := tx.InsertAllocation(txCtx, allocation); err != nil {
			return err
		}
		if err := c.outbox(txCtx, tx, "supply.allocated", "allocation", allocation.ID, allocation); err != nil {
			return err
		}
		return c.audit(txCtx, tx, cmd.Actor, "supply.allocate", "allocation", allocation.ID, "success", fmt.Sprintf("quantity=%d", cmd.Quantity))
	})
	if err != nil {
