...
func (c *Coordinator) ActivateIncident(ctx context.Context, cmd domain.ActivateIncidentCommand) (domain.Incident, error) {
	...
	txCtx := uncancelledContext(ctx)
	err := c.repo.WithinTx(txCtx, func(tx repository.Tx) error {
		warning, err := tx.GetWarning(txCtx, cmd.WarningID)
		if err != nil {
			return err
		}
		...
		if err := tx.InsertIncident(txCtx, incident); err != nil {
			return err
		}
		...
		for _, regionID := range cmd.Regions {
			...
			if err := tx.InsertResponse(txCtx, response); err != nil {
				return err
			}
		}
		if err := c.outbox(txCtx, tx, "incident.activated", "incident", incident.ID, incident); err != nil {
			return err
		}
		if err := c.job(txCtx, tx, "response.check", incident.ID, map[string]any{"incident_id": incident.ID}, now.Add(15*time.Minute)); err != nil {
			return err
		}
		return c.audit(txCtx, tx, cmd.Actor, "incident.activate", "incident", incident.ID, "success", cmd.Name)
	})
	if err != nil {
